...
Initial system was built on top of an existing project named as EmailValet\footnote{https://www.gmailvalet.com/} also known as GmailValet in \ac{HCI} group\footnote{https://hci.stanford.edu/} at Stanford University. EmailValet is a \ac{SaaS} application that combines email client with task manager. Remote assistants from an expert crowdsourcing marketplace extract tasks from incoming emails, and annotate them in a task list separated from the inbox \citep{Kokkalis2013}.
\vspace{1cm}
...
EmailValet requires an Gmail account to register and sign in to the system, therefore application user has to grant access to EmailValet to use it. Figure \ref{fig:GmailValetInbox} shows EmailValet's inbox view after user signed in and browse to a campaign\footnote{In this thesis, the word campaign is used as a shorhand for a mass email communication campaign, where a researcher initiates a conversation with gorups of people related to one specific topic.}. On the left hand side, there is the email list pane, and selected email can be read on the reading pane, at right.
...
Starting a campaign is done exactly the same way as in composing an email. However, the corresponding view has additional two input fields to get the campaign and the template names from the user. Campaign name will help to identify the groups of conversations from other campaigns, or regular email conversations in the inbox. Template name will help us to identify which respondents's answers correspond to which email that we sent. For example, in figure \ref{fig:GmailValetInbox}, a recipient's answer is a response to the questionnaire invitation that we sent earlier. A template name will also help us to represent a template in a tree structure to pick from to reuse, and show the state of the conversation in the same tree structure.
\vspace{1cm}

Researcher can add their recipients list into the Merge\footnote{The word "merge" comes from the term "mail merge", which means a procedure to enable you to combine a document with data files which consists a list of names, therefore copies of the document will be different for each person it is sent to \citep{CollinsEnglishDictionary2013}.} input field as in figure \ref{fig:ComposePaneWithTemplates}, which corresponds to "To" field in a regular email client. However, the format should be as in the figure, that is first name, last name, and email address in angle brackets. Hence, those fields can be used dynamically in the salutation of the emails by writing \textit{\{\{first\_name\}\}} and \textit{\{\{last\_name\}\}} variables. Once those recipients list is entered and the email is sent, those contacts will be recorded in the Gmail's contacts book.
\vspace{1cm}
...
Figure \ref{fig:ComposePaneWithTemplates} also shows the template tree to show the state of the conversation, and allow users to select template from. The indentation at the nodes of the tree helps to identify which templates are used after which template, and therefore it gives an idea about the current state of the conversation. According to this sample scenario in the figure, After the invitation email, there were two possible answers that we gave to the recipients according to their response to out invitation. For those who who rejected our invitation, we can select the "Rejected" node from the tree to reuse that answer that we gave before. This can be a motivation email as discussed in section \ref{subsec:4.1.2:ImprReduEffo}. At the parallel, the conversation was continued with the people who accepted the invitation, and we continued to send \ac{NDA}, and afterwards questionnaire. After we sent the questionnaire, there were cases, where respondents returned back with an email to ask some questions to clarify some points in the questionnaire that we sent. According to figure \ref{fig:ComposePaneWithTemplates}, these cases were the questions number 3 and 18, and we explained those two questions from the questionnaire. Again, since the system allow us to reuse the templates, we don't need to rewrite our explanation if those questions were not clear for many people. On the other hand, if researchers want to create a new template, he or she can simply add the name of the template into corresponding input fields, and send the email. Next time, the researcher will find it in the template tree under the corresponding level of the node. Also, it is possible to select a template, made some modification and save it as a new template. Therefore, a slightly different versions of template can be reused during the communication.
\vspace{1cm}

Finally, there is an option to add \ac{KVP}s while reading respondents' answers. In the reading pane, two input fields are offered to enter a key and its value to attach them to the email that is being read. As in figure \ref{fig:EMailKVPs}, while user can add new \ac{KVP}s, they can also see the existing ones attached to that email message.
...
In conclusion, while the prototype satisfied many of the initial requirements that was discussed in section \ref{subsec:4.2.1:ProtReqiAnal}, there were some drawbacks that will be mentioned in the section \ref{sec:4.3:EvalProt} why it was discarded instead of going into the production code. But before that, we will go into some implementation details about EmailValet's mass email communication modifications.

\subsection{Architecture}
\label{subsec:4.2.3:ProtArch}

Even though, we had a ready-to-use email client with EmailValet, there were quite bit changes to reflect the requirements that was mentioned in section \ref{subsec:4.2.1:ProtReqiAnal}, and since EmailValet was dealing with single email messages not a group of them as a email client, there were also some modifications to make the prototype understand whether a message belongs to a campaign or not. In this section, we will briefly see how the changes for prototype fit into existing EmailValet's design, and a technology overview to understand how mass email communication is done.

\paragraph{Identification of a Campaign Message}
An email message consists header and body sections as defined by RFC 5322. While a message's header keeps structured information such as "From", "To", "\ac{CC}", Subject, Date, and other information with a special syntax, a message's body consists the content, which is an unstructured text and optional to have \citep{rfc5322}. An example email message with its header fields and body can be found in Appendix \ref{app:EmaiMessForm}.
\vspace{1cm}

One of the fields in an email message's header is called Message-ID\footnote{The message identifier, Message-ID, is enclosed in the angle bracket characters, "<" and ">", an its syntax only permits the dot-atom-text form, which is \verb|1*atext *("." 1*atext)|, on the left-hand side of the "@" and a domain name is recommended for the right-hand side of the "@" \citep{rfc5322}. For example: \verb|<CAF2E4bfH4+GAYHcJFZJ6dTJJ+pux4mTjff2neCS_VR_zVCUY9g@mail.gmail.com>|}, which is a unique message identifier, and set by email clients. A Message-ID is used in another header field, named In-Reply-To, when a message is a reply to that message.
\vspace{1cm}

EmailValet had already the data model to store an email message's related data when it is synchronized with Gmail. However, creating a mass email campaign requires to identify if the synchronized email belongs to a mass email campaign that was created or if it is an answer to a campaign message that was sent. Because, EmailValet's initial design were able to fetch emails from Gmail, then assign properties afterwards. Even the emails composed by EmailValet were stored after they got replies from their recipients. Therefore, Message-ID and In-Reply-To fields of an email message are leveraged to keep track of a campaign message by setting a Message-ID of it, and storing in a Campaign data model as in figure \ref{fig:UML_Prototype}. Hence, we were able to identify the messages during the synchronization with Gmail whether a message is an initial campaign message, or if it is an answer to a campaign message that was created.

\begin{figure}[htbp]
	\centering
	\includegraphics[width=1.00\textwidth]{imgs/UML_Draw_Prototype.png}
	\caption[Model Dependency of the New Features]{Model Dependency of the New Features}
	\label{fig:UML_Prototype}
\end{figure}

\paragraph{Template and Email Message Relation} Each campaign message that is composed in EmaiValet is actually a template with dynamic variables in it. Therefore, the campaign messages that is fetched from Gmail corresponds to a template in EmailValet. This relation can be seen in figure \ref{fig:UML_Prototype}.
-- burda TRee den de bahset, text olur cok guzel
...
--> FOOTNOTE: synchronization is actually IMAP call
...
In this section, we will analyze what are the drawbacks of the prototype, even though it fulfilled most of the requirements mentioned in \ref{subsec:4.2.1:ProtReqiAnal}. Some of these drawbacks were related with continuing to development of the prototype on an existing application whose main purpose was different than a regular email client.
...
--> KVPs are about one email, not about a contact
- last 100 email
- we don't need a whole inbox, we don't email to replace your email client, but come and use us when you wanna create campaigns
- it slow cause of above two things
- if no name or surname, email will have blank placeholders
