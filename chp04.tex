...
Even though, we had a ready-to-use email client with EmailValet, there were quite bit changes to reflect the requirements that was mentioned in section \ref{subsec:4.2.1:ProtReqiAnal}, and since EmailValet was dealing with single email messages not a group of them as a email client, there were also some modifications to make the prototype understand whether a message belongs to a campaign or not. In this section, we will briefly see how the changes for the prototype fit into existing EmailValet's design, and make a technology overview to understand how mass email communication is done.
...
Nested set model helps us to find a node's children faster than a usual agency set model, where each node only keeps information about its parent node \citep[pages 45--47]{Celko2004}. For example, to find all the nodes of the node named "Albert" in figure \ref{fig:NestedSetModelTree}, we just need to find all nodes whose left value is greater than 1, and right value is less than 12. In adjacency list, however, first we need to find all the nodes whose parent id is equal to "Albert"s id, then for those nodes' ids, we need to find other nodes whose parent id is equal to them, and so until all nodes are finished in the tree.

Template data model in \ref{fig:UML_Prototype} keeps those left and right number information on one column named "ancestry", which is populated by a gem\footnote{A Gem or a RubyGem is a software package, containining a packaged Ruby application or library \citep{RubyGemsGuides2013}.} named Ancestry\footnote{https://github.com/stefankroes/ancestry}.

\paragraph{\ac{KVP}s Relation with a Campaign and a Message}
As in figure \ref{fig:UML_Prototype}, each email message can have more than one value that corresponds to a key of \ac{KVP}. This allows us identify, when a \ac{KVP} added to a message belonging to a respondent. Key are organized according to campaigns, therefore a same key name can be used in different campaigns for different purposes. This is mainly because a word chosen as a key can have different meanings depending on context.
\vspace{1cm}

After getting an overview of the technical implementation of mass Emil communication feature on EmailValet, next section will focus on what was missing on this initial design, and where were the problems on that.
