...
After we start getting answers from respondents, we might extract some information from those email email. For example, this could be a date that we need to return to the recipients with a reminder. Storing those information as we stored contact information might allow us easy to access them again, and gives an opportunity to reuse them in the email content as a dynamic variable.
\vspace{1cm}

We mentioned about the reusability of contact information, but after the initial invitation email, there will be many cases to reuse different version of the email for further state of the conversations. We mentioned the term "template" as initial email consists some dynamic variable, and after those variables got their actual values, it will be the actual message that we want to send. Here, we will add another meaning to the template to emphasize a message's reusability. Let us consider the "maybe" case that was mentioned to represent respondents who were busy to participate in the questionnaire at the moment, but maybe it is possible to participate in later (See figure~\ref{fig:EmailDiagram}). 
